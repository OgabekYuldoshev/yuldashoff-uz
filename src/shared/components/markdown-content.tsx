...
const MARKDOWN_COMPONENTS: Components = {
	...
	h2: ({ children, className, ...props }) => (
		<h2
			{...props}
			className={cn(className, "mb-3 text-3xl font-semibold tracking-tight")}
		>
			{children}
		</h2>
	),
	h3: ({ children, className, ...props }) => (
		<h3
			{...props}
			className={cn(className, "mb-2 text-2xl font-semibold tracking-tight")}
		>
			{children}
		</h3>
	),
	...
	// Authored content carries no intrinsic dimensions, so `next/image` cannot be
	// used here without guessing a size for every image in every document.
	...
	code: ({ children, className, ...props }) => (
		<code
			{...props}
			className={cn(className, "rounded px-1 py-0.5 font-mono text-sm")}
		>
			{children}
		</code>
	),
